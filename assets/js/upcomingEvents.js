...
function newArray(array){
    ...
    for (let dateComming of array){
        if (dateComming.date>data.currentDate){
...
function imprCard(data,elementHtml){
 ...
 imprCard(upcomingData,containerCards)

//checkbox
let divCheck = document.getElementById("div-check") 

let category = upcomingData.map(category => category.category)
...
function filterCards(events,eventsFilterStrings){
  ...
  let cardsSelect=filterCards(upcomingData,checkSelect)
  empty(cards)
      if(checkSelect.length==0){
          return imprCard(upcomingData,cards,newCard)
      }else{
